...
func attacksFixturePath(t *testing.T, rel string) string {
	...
	// full/ is at src/edge/netpen/full/; attacks/testdata is at src/edge/netpen/attacks/testdata
	dir := wd
	for {
...
// The fixture is known to carry a summary frame, so absence here is a
// real decode failure, never a skip.
func TestParseReconFrame_VTP(t *testing.T) {
	pkts := readPcap(t, attacksFixturePath(t, "l2/vtp.pcap"))
	// Scan forward to the summary frame: the fixture holds several VTP
	// codes and the summary carries the domain and revision.
	ev := Evidence{}
	for _, p := range pkts {
		parseReconFrame(ev, p)
		if ev.Has(EvVTPDomain) || ev.Has(EvVTPRev) {
			return
		}
	}
	t.Fatal("VTP summary frame did not set domain/revision evidence")
}
...
func TestParseReconFrame_MVRP(t *testing.T) {
	...
	t.Fatal("MVRP fixture frames did not set mvrp evidence")
}
