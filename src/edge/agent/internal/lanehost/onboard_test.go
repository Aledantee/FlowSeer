...
type listerFake struct {
	mu       sync.Mutex
	listings [][]*apiedgev1.ListedDevice
	calls    int
	err      error
}

func (l *listerFake) ListDevices(
	_ context.Context, _ *connect.Request[apiedgev1.ListDevicesRequest],
) (*connect.Response[apiedgev1.ListDevicesResponse], error) {
	l.mu.Lock()
	defer l.mu.Unlock()
	l.calls++
	if l.err != nil {
	...
	return connect.NewResponse(apiedgev1.ListDevicesResponse_builder{Devices: batch}.Build()), nil
}
...
func (r *endpointRecorder) openSNMP(endpoint lanehost.Endpoint) func(context.Context, *apiedgev1.DeviceCredential) (access.SNMPSession, error) {
	r.mu.Lock()
	r.snmp = append(r.snmp, endpoint)
	r.mu.Unlock()
	return func(context.Context, *apiedgev1.DeviceCredential) (access.SNMPSession, error) {
		return access.SNMPSession{}, nil
	}
}

func (r *endpointRecorder) openShell(endpoint lanehost.Endpoint) func(context.Context, *apiedgev1.DeviceCredential, string) (access.ShellSession, error) {
	r.mu.Lock()
	r.shell = append(r.shell, endpoint)
	r.mu.Unlock()
	return func(context.Context, *apiedgev1.DeviceCredential, string) (access.ShellSession, error) {
...
func listedDevice(deviceID string, horizon time.Duration) *apiedgev1.ListedDevice {
	listed := apiedgev1.ListedDevice_builder{
...
func TestTheAgentOnboardsWhatItIsToldToServe(t *testing.T) {
	lister := &listerFake{listings: [][]*apiedgev1.ListedDevice{{
...
func TestTheSessionFactoriesAreBuiltForWhereTheDeviceAnswers(t *testing.T) {
	...
	lister := &listerFake{listings: [][]*apiedgev1.ListedDevice{{listedDevice(deviceOne, time.Minute), withPorts}}}
...
func TestADeviceListedWithNoHorizonIsStillOnboarded(t *testing.T) {
	lister := &listerFake{listings: [][]*apiedgev1.ListedDevice{{listedDevice(deviceOne, 0)}}}
	registrar := newRegistrar()
	logs := &recordingLogs{}
...
func TestARelistIsSilentAboutADeviceThatDidNotChange(t *testing.T) {
	lister := &listerFake{listings: [][]*apiedgev1.ListedDevice{{listedDevice(deviceOne, 30*time.Second)}}}
	logs := &recordingLogs{}
