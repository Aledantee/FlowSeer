...
// phase.
func (r laneReporter) Reported(ctx context.Context, deviceKey string, result *dispatchv1.ExecuteResult) {
	r.send(ctx, deviceKey, func(report *dispatchv1.ReportRequest) { report.SetResult(result) })
}
...
func (r laneReporter) CheckpointAcked(ctx context.Context, deviceKey string, ack *dispatchv1.CheckpointAck) {
	r.send(ctx, deviceKey, func(report *dispatchv1.ReportRequest) { report.SetCheckpointAck(ack) })
}

// HoldResolvedAcked carries the acknowledgement of a hold central cleared.
func (r laneReporter) HoldResolvedAcked(ctx context.Context, deviceKey string, ack *dispatchv1.HoldResolvedAck) {
	r.send(ctx, deviceKey, func(report *dispatchv1.ReportRequest) { report.SetHoldResolvedAck(ack) })
}
...
func (r laneReporter) Onboarded(ctx context.Context, deviceKey, fingerprint string) {
	...
	r.send(ctx, deviceKey, func(report *dispatchv1.ReportRequest) { report.SetOnboarded(onboarded) })
}

func (r laneReporter) send(ctx context.Context, deviceKey string, set func(*dispatchv1.ReportRequest)) {
