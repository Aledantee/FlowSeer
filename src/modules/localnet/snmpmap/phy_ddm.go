...
func walkDlinkModules(ctx context.Context, sess snmp.Session, facets map[uint32]*phyv1.EthernetFacet) error {
	...
	for _, row := range sfpWalk.Iter() {
		mapDlinkSfp(facets, uint32(row.Key.IfIndex), row)
	}
	...
	for _, row := range ddmWalk.Iter() {
		mapDlinkDdm(facets, uint32(row.Key.IfIndex), row)
	}
...
func walkHpModules(ctx context.Context, sess snmp.Session, facets map[uint32]*phyv1.EthernetFacet) error {
	...
	for _, row := range walk.Iter() {
		mapHpXcvr(facets, uint32(row.Key.IfIndex), row)
	}
...
func walkHh3cModules(ctx context.Context, sess snmp.Session, facets map[uint32]*phyv1.EthernetFacet) error {
	...
	for _, row := range infoWalk.Iter() {
		mapHh3cXcvr(facets, uint32(row.Key.IfIndex), row)
	}
	...
	for _, row := range chanWalk.Iter() {
		channel := row.Key.Hh3cTransceiverChannelIndex
		if channel <= 0 {
			continue
		}

		mapHh3cChannel(facets, uint32(row.Key.IfIndex), uint32(channel), row)
	}
