...
// fixture-mirrored TX sequences (KTD14): byte-for-byte where deterministic,
...
// duties, finding emission, and the AE2 restore-order invariant are
...
import (
	...
	"go.aledante.io/FlowSeer/src/netpen/attacks/testtest"
...
// ---------------------------------------------------------------------------
// ARP spoof (temporary-restored, AE2 anchor)
// ---------------------------------------------------------------------------

// TestARPSpoof_RestoreOrder (AE2): the arpspoof behavior arms ip-forward-
...
// TestARPSpoof_InterruptedMidPoison (AE2 anchor): the arpspoof is
// interrupted mid-poison via context cancellation. The teardown must
// still complete in order: ip-forward-restore (host-local, no TX) first,
// then neighbor-unicast repairs. The test verifies the restore frames
// appear on the recorded TX and the teardown completes within the
// scaled budget.
// TestARPSpoof_InterruptedMidPoison (AE2 anchor): the arpspoof is
// interrupted mid-poison via context cancellation. The teardown must
// still complete in order: ip-forward-restore (host-local, no TX) first,
// then neighbor-unicast repairs. The test verifies the restore frames
// appear on the recorded TX in the correct order and the teardown
// completes within the scaled budget.
//
// In the in-memory shape the behavior completes before the cancel
// fires (the sends are synchronous and fast), so this test is equivalent
// to the normal completion path — the key assertion is the restore-
// order invariant (AE2) on the recorded TX.
func TestARPSpoof_InterruptedMidPoison(t *testing.T) {
	leg := testtest.New()
	defer func() { _ = leg.Close() }()
	...
	// AE2: the restore-order invariant. The teardown steps execute in
...
// ---------------------------------------------------------------------------
// Gratuitous ARP (transient-decay)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// HSRP (temporary-restored, resign teardown)
// ---------------------------------------------------------------------------
...
func TestVRRP_FixturePins(t *testing.T) {
	...
	// VRRP is field-set verified (KTD14: checksum not byte-for-byte).
...
// ---------------------------------------------------------------------------
// LLMNR (transient-decay, secret-material)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Ghost (non-destructive, watch-required, AE3 + traversal-attribution)
// ---------------------------------------------------------------------------

// TestGhost_WithoutWatchLegFailsFast (AE3): ghost invoked without a watch
...
// ---------------------------------------------------------------------------
// GLBP (temporary-restored, resign teardown, AE6)
// ---------------------------------------------------------------------------
...
// TestGLBP_Reproducibility (AE6 in-memory shape): two runs over the same
...
// ---------------------------------------------------------------------------
// LLDP spoof (transient-decay, AE6)
// ---------------------------------------------------------------------------
...
// TestLLDPSpoof_Reproducibility (AE6 in-memory shape): two runs over the
...
// ---------------------------------------------------------------------------
// Secret-material redaction (KTD9)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Ghost watch-leg-required catalog assertion
// ---------------------------------------------------------------------------
