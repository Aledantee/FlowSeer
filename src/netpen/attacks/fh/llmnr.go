...
//
// Secret-material (KTD9): if the attacker observes authentication traffic
// directed to it (victims send SMB/HTTP to the spoofed IP), any captured
// credential is emitted as length+protocol only via [findings.NewSecret].
...
func RunLLMNR(ctx context.Context, deps runner.Deps) error {
	...
	// would be emitted via findings.NewSecret (KTD9): protocol + length
	// only, never the value. The behavior-level test asserts the secret
	// shape.
	//
