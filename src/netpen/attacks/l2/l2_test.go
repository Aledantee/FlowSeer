...
// sequences (KTD14): byte-for-byte where deterministic, field-set where
...
// emission, and KTD12 restore ordering are asserted via the recorded TX.
...
func TestDTP_NegotiateAndRestore(t *testing.T) {
	...
	// KTD12: the behavior sends the desirable frame, then the teardown
...
// TestEtherChannel_Reproducibility (AE6 in-memory shape): two runs over
...
// ---------------------------------------------------------------------------
// Catalog class assertions (gate behavior)
// ---------------------------------------------------------------------------
...
func TestVTPWipeWithoutAckRefused(t *testing.T) {
	...
	// No frames should have been sent (AE1: gate evaluated before TX).
	if leg.SendCount() != 0 {
