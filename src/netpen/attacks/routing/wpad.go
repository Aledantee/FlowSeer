...
// gone, so no explicit teardown is armed (R14: transient-decay has no
...
// (netpen/wpad-port-collision) rather than crashing (U11 edge scenario).
//
// Secret-material (KTD9): credentials harvested by the rogue proxy
// (e.g. NTLM hashes from proxy authentication) are emitted as
...
func RunWPAD(ctx context.Context, deps runner.Deps) error {
	...
	// --- Start the embedded PAC proxy ---
	state, err := startWPADProxy()
	if err != nil {
		// Port collision: named coded error, not a crash (U11).
		return errs.New().
	...
	// the finding — only length + protocol (KTD9).
