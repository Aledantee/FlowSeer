...
// registrations.go holds the full 35-behavior registration table,
// transcribed from the plan's authoritative "Durability classification of
// all 35 behaviors" table (High-Level Technical Design). Every R1 command
// name and the 8 superset attacks are registered here with the
// correctness commitment: each (behavior, mode) pair's class, legs, and
// teardown match the plan's oracle row-for-row.
//
// These registrations are metadata only — the behavior run functions are
// later units (U8-U11). U8-U11 will add concrete behavior registrations in
// their own attack packages via [Register]; this file registers the
// baseline metadata skeleton so the catalog, generator, and tests are
// exercised from U4.
...
func init() {
	...
	// ---- portsteal: default transient-decay, --relay temporary-restored ----
	Register(Behavior{
