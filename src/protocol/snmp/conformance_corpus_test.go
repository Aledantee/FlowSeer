...
var conformanceCorpus = []corpusRow{
	{ID: "enc-counter64-v1", Clause: "RFC 2576 §3", Provenance: "gosnmp (no check)", Behavior: "Counter64 in a v1 response: decode + typed warning (data preserved), never panic/type-confusion", Adversarial: "SNMPv1 message carrying a Counter64 varbind", Status: statusCovered},
	{ID: "enc-zerolen-int", Clause: "X.690 §8.3.1", Provenance: "gosnmp #241", Behavior: "zero-length signed INTEGER (02 00) errors; zero-length unsigned counter tolerated as 0 (recorded leniency)", Adversarial: "zero-length signed INTEGER TLV (02 00)", Status: statusCovered},
	{ID: "enc-nonminimal-int", Clause: "X.690 §8.3.2", Provenance: "gosnmp #371", Behavior: "encode minimal (-1 -> FF); decode tolerates non-minimal", Adversarial: "non-minimal 4-octet INTEGER 0xFFFFFFFF on decode", Status: statusCovered},
	{ID: "enc-maxrep-signed", Clause: "RFC 3416", Provenance: "gosnmp #293", Behavior: "GETBULK max-repetitions 128-255 encode unsigned, not negative", Adversarial: "GETBULK with max-repetitions 128/200/255 (high-bit byte)", Status: statusCovered},
	{ID: "enc-id-range", Clause: "RFC 3412", Provenance: "gosnmp #272", Behavior: "msgID/request-id stay in 0..2^31-1, no overflow to negative", Adversarial: "request-id at 2^31-1 (math.MaxInt32) round-tripped through encode/decode", Status: statusCovered},
	{ID: "enc-ipaddr-longform", Clause: "X.690 §8.1.3", Provenance: "gosnmp #544", Behavior: "long-form BER length on IpAddress (40 81 04 ..) decodes to the 4-byte address", Adversarial: "IpAddress TLV with long-form length (40 81 04 ..) wrapping a 4-octet address", Status: statusCovered},
	{ID: "enc-ipaddr-8byte", Clause: "RFC 2578 §7.1.5", Provenance: "gosnmp #544", Behavior: "IpAddress with 8 content bytes -> typed error, not panic (no sane IP)", Adversarial: "IpAddress TLV with 8 content octets", Status: statusCovered},
	{ID: "enc-opaque-unknown", Clause: "RFC 2856", Provenance: "gosnmp #374", Behavior: "unknown Opaque sub-type (0x7a) -> raw bytes, not nil", Adversarial: "Opaque value with unknown wrapped-real marker (9F 7A ..)", Status: statusCovered},
	{ID: "enc-opaque-zero", Clause: "RFC 2856", Provenance: "gosnmp #453", Behavior: "Opaque float/double 0.0 -> full 4/8-byte IEEE-754, not truncated", Adversarial: "Opaque float/double 0.0 (all-zero IEEE-754 payload)", Status: statusCovered},
	{ID: "enc-unsigned-as-signed", Clause: "RFC 2578 §7.1.6", Provenance: "Check Point sk115119; IBM IZ77427", Behavior: "INTEGER-tagged (0x02) value with MSB set decodes to a negative Integer32Var; coercion to uint32 is rejected as ErrLossyConversion (reject, not reinterpret)", Status: statusAcceptedRisk, Accepted: "A negative INTEGER-tagged value is indistinguishable from a genuine -1; rejecting (the MikroTik leniency precedent) is safer than silently reinterpreting it as ~4 billion. Reinterpretation would weaken sign-error detection. Pinned by TestEncUnsignedAsSigned_RejectsNegativeIntTagged."},
	{ID: "enc-neg-length", Clause: "X.690 §8.1.3", Provenance: "gosnmp #552", Behavior: "length byte sign-extending to negative int64 -> no negative-index panic (named fuzz seed)", Adversarial: "TLV with long-form length octet 0x88 (04 88 7f ff ..) announcing 8 length octets", Status: statusCovered},
	{ID: "enc-dateandtime-lens", Clause: "RFC 2579", Provenance: "snmp_exporter #321", Behavior: "DateAndTime 0-byte (unknown) and 7/other-byte variants handled distinctly", Adversarial: "DateAndTime OCTET STRING of 0 and 7 (and other off-) octets", Status: statusCovered},
	{ID: "enc-bits-padding", Clause: "RFC 2579", Provenance: "RFC 2579", Behavior: "BITS with trailing zero padding / short value tolerated", Adversarial: "BIT STRING with trailing zero padding, short, and empty content", Status: statusCovered},
	{ID: "enc-timeticks-range", Clause: "RFC 2578", Provenance: "RFC 2578", Behavior: "TimeTicks 5-byte unsigned / out-of-range masked to 32 bits (scoped to TimeTicks)", Adversarial: "5-octet TimeTicks content 0x01_00_00_00_2A (2^32+42)", Status: statusCovered},
	{ID: "enc-v1trap-spectrap", Clause: "RFC 2576", Provenance: "gosnmp #182", Behavior: "SNMPv1 Trap specific-trap > 127 not byte-truncated in v1->v2c translation", Adversarial: "SNMPv1 enterpriseSpecific trap with specific-trap=200", Status: statusCovered},

	{ID: "walk-toobig-fallback", Clause: "RFC 3416", Provenance: "MikroTik(>50), Cisco/IOS-XR, Nokia, F5", Behavior: "tooBig -> halve max-repetitions -> fall back to GETNEXT-per-OID; full table, no abort", Adversarial: "agent returning tooBig above a max-rep threshold, and one rejecting GetBulk at every max-rep", Status: statusCovered},
	{ID: "walk-mid-pdu-eomv", Clause: "RFC 3416", Provenance: "dense carrier tables", Behavior: "single-chain GETBULK: yield all preceding values, terminate at the EndOfMibView varbind (no data loss)", Adversarial: "GETBULK chain of N values followed by EndOfMibView in one PDU", Status: statusCovered},
	{ID: "walk-nosuch-semantics", Clause: "RFC 3416", Provenance: "RFC 3416", Behavior: "noSuchInstance = skip & continue (advance cursor); noSuchObject on subtree root = abort that subtree; classified before the cycle guard", Adversarial: "MIB with noSuchInstance and noSuchObject varbinds interleaved with values", Status: statusCovered},
	{ID: "walk-mutate-midwalk", Clause: "RFC 3416", Provenance: "Ruckus/Aruba/UniFi WLAN", Behavior: "rows added/dropped mid-walk -> self-consistent snapshot tolerating index gaps + duplicate indices, no loop/error", Adversarial: "responder dropping and adding rows between successive GETBULK PDUs (maxPerPDU forces multi-PDU)", Status: statusCovered},
	{ID: "walk-cycling-oid", Clause: "RFC 3416", Provenance: "gosnmp #401 Juniper; Cisco CSCuf16921", Behavior: "non-increasing/cycling OIDs -> bounded skip-forward, no infinite loop/OOM; cycle distinct from single regress", Adversarial: "scripted GetNext returning an exact-repeat OID, and a non-increasing-then-forward sequence", Status: statusCovered},
	{ID: "walk-leaf-start", Clause: "RFC 3416", Provenance: "gosnmp #170", Behavior: "walk starting on a leaf OID returns the next OID, not nothing", Adversarial: "BulkWalk rooted at a scalar leaf node (sysDescr) with only the .0 instance present", Status: statusCovered},
	{ID: "walk-large-value-hang", Clause: "RFC 3416", Provenance: "gosnmp #408", Behavior: "BulkWalk must not hang on a varbind with a >1KB OctetString value", Adversarial: "table row carrying a 4096-byte OctetString value", Status: statusCovered},
	{ID: "txp-dup-response", Clause: "RFC 3416", Provenance: "gosnmp #417", Behavior: "duplicate/retransmitted response dropped by request-id demux; later genuine reply still resolves", Adversarial: "agent emitting a late duplicate of an already-delivered reply", Status: statusCovered},
	{ID: "txp-subtree-exit", Clause: "RFC 3416", Provenance: "RFC 3416", Behavior: "returned OID outside requested subtree prefix -> normal (non-error) termination", Adversarial: "agent returning an OID in a sibling column outside the requested subtree", Status: statusCovered},

	{ID: "usm-authbit-bypass", Clause: "RFC 3414 §3.2", Provenance: "gosnmp #496", Behavior: "auth/priv downgrade matrix: cleared auth/priv bit on a configured session -> ErrUSMDowngrade before HMAC; Report authNoPriv-on-authPriv allowed", Adversarial: "replies with the auth bit cleared at authNoPriv/authPriv and the priv bit cleared at authPriv; plus an authNoPriv Report on an authPriv session", Status: statusCovered},
	{ID: "usm-report-randomid", Clause: "RFC 3414 §4", Provenance: "gosnmp #139", Behavior: "unsolicited/mismatched-id Report dropped-and-counted, never aborts a waiter; genuine reply still resolves", Adversarial: "agent injecting an authenticated Report with a mismatched msgID before the genuine reply", Status: statusCovered},
	{ID: "usm-aes-keyext", Clause: "RFC 3826; draft-reeder", Provenance: "gosnmp #424; Cisco/Extreme", Behavior: "AES-192/256 Blumenthal vs Reeder (C) key-extension vectors; distinct AES192 vs AES192C keys (unit-level oracle; no net-snmp cell)", Adversarial: "external pysnmp/hashlib vectors for AES-192/256 across MD5/SHA auth, Blumenthal vs Reeder", Status: statusCovered},
	{ID: "usm-keycache-passphrase", Clause: "RFC 3414 §2.6", Provenance: "gosnmp #424", Behavior: "no passphrase-keyed cache exists; keys derived per (engineID,user) from passphrase material", Status: statusAcceptedRisk, Accepted: "There is no passphrase-keyed key cache, so the failure mode the quirk guards against (a stale key returned for a changed priv passphrase) cannot occur. Grounded by TestUSM_KeyDerivationPerPrivPassphrase: differing priv passphrases derive different keys."},
	{ID: "usm-3step-discovery", Clause: "RFC 3414 §4", Provenance: "gosnmp #511", Behavior: "initial discovery performs the authenticated boots/time resync before the first real request", Adversarial: "empty-EngineID session forced through probe -> unknownEngineID Report -> authenticated request", Status: statusCovered},
	{ID: "usm-trap-reportable", Clause: "RFC 3412 §6.4", Provenance: "gosnmp #391", Behavior: "reportable-flag handling correct on received v3 traps vs informs", Adversarial: "decoded v3 trap (reportable clear) vs v3 inform (reportable set); tampered trap dropped with no Report", Status: statusCovered},

	{ID: "raw-wrong-typed-column", Clause: "RFC 2578 §7.1.6", Provenance: "telegraf #14598; snmp_exporter #338 (proprietary/buggy agents reporting types diverging from the MIB declaration)", Behavior: "column value whose wire tag diverges from the MIB-declared Kind: the fused arm declines (ok=false, never an error) and the generic decoder's coercion rules apply — values and errors identical to the pre-R25 path", Adversarial: "walk where a Counter32-declared column arrives Gauge32-tagged (coerces) and an Integer32-declared column arrives OctetString-tagged (typed mismatch error)", Status: statusCovered},
	{ID: "raw-noncanonical-oid-arc", Clause: "X.690 §8.19.2", Provenance: "chemist/snmp #17 (agents emitting BER that is valid but not shortest-form)", Behavior: "response name OID carrying a zero-padded (0x80-prefixed) sub-identifier: mirror validation refuses raw delivery and the read loop decodes eagerly — the walk yields identical data via pre-decoded varbinds, and the byte-order walk guards never see a non-canonical arc", Adversarial: "GetResponse datagram whose varbind name encodes a sub-identifier with a redundant leading 0x80 continuation octet", Status: statusCovered},

	{ID: "usm-timewindow-rollback", Clause: "RFC 3414 §2.2.3", Provenance: "deepening (security-lens)", Behavior: "polling-side engineBaseline.update rejects a boots/time pair that would decrease boots or move time backward", Adversarial: "resync Report with lower engineBoots, and backward engineTime at the same boots", Status: statusCovered},
	{ID: "usm-msgid-predictability", Clause: "RFC 3412", Provenance: "deepening (security-lens)", Behavior: "msgID drawn fresh from the CSPRNG per message (not last+1), removing prediction in the unauthenticated window", Adversarial: "sample of consecutive msgIDs checked for previous+1 sequentiality", Status: statusCovered},
	{ID: "usm-inform-timewindow", Clause: "RFC 3414 §3.2", Provenance: "deepening (security-lens)", Behavior: "authoritative-role authoritativeTimeOK enforces the ±150s engineTime window + post-restart quarantine; int32 boundary safe (int64 diff)", Adversarial: "informs at window edges, during quarantine, wrong boots, and forged math.MinInt32/MaxInt32 engineTime", Status: statusCovered},
	{ID: "usm-authoritative-boots-pinned", Clause: "RFC 3414 §3.2", Provenance: "deepening (security-lens)", Behavior: "authoritativeBoots pinned to 2^31-1 -> §3.2 boots-sequence check disabled for the listener role; quarantine reduces but does not close cross-restart inform replay", Status: statusAcceptedRisk, Accepted: "authoritativeBoots is pinned to 2^31-1 to avoid cross-restart persistent state; the §3.2 boots-sequence check is therefore disabled for the listener role and the time window is the sole gate. The post-restart quarantine (150s) closes the immediate replay window but not arbitrary pre-restart replays. Full closure needs persistent boots storage. Grounded by TestUSM_AuthoritativeBootsPinned."},
}
...
func renderConformanceMatrix(rows []corpusRow) string {
	...
	b.WriteString("run `UPDATE_CONFORMANCE=1 go test ./src/protocol/snmp/ -run TestConformanceMatrixUpToDate`.\n\n")
...
func TestConformanceMatrixUpToDate(t *testing.T) {
	...
	if string(got) != want {
		t.Errorf("CONFORMANCE.md is stale — run `UPDATE_CONFORMANCE=1 go test ./src/protocol/snmp/ -run TestConformanceMatrixUpToDate`")
	}
}
