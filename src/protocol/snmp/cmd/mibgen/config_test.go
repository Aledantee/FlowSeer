...
// nineteen modules with unique names/packages: sixteen with tables and
// scalars, plus the three identity-only vendor product MIBs.
func TestLoadConfig_FullSchema(t *testing.T) {
	...
	if got, want := len(cfg.Modules), 19; got != want {
	...
	if cfg.Modules[len(cfg.Modules)-1].Name != "HH3C-PRODUCT-ID-MIB" {
		t.Errorf("last module = %q; want HH3C-PRODUCT-ID-MIB", cfg.Modules[len(cfg.Modules)-1].Name)
	}
}
