...
| `nc-t4-identity` | device identity reads | IOS-XE lab device (pending) | hostname, serial, model, and OS version return as typed values via the generated native and device-hardware bindings | pending |
| `nc-t4-invalid-rollback` | rejected candidate cleanup | IOS-XE lab device (pending) | a staged invalid candidate change is rejected, discarded, and unlocked; read-back diff proves running unchanged | pending |
| `nc-t4-reversible-edit` | reversible config edit | IOS-XE lab device (pending) | a reversible edit round-trips through candidate/commit and its revert, both proven by read-back | pending |
| `nc-t4-interface-walk` | typed interface state walk | IOS-XE lab device (pending) | the interface-state Walker completes with typed rows on hardware | pending |
| `nc-t4-watch-induced` | interface change detection | IOS-XE lab device (pending; operator-induced toggle) | an interface state change between ticks emits exactly one Modified for that row on hardware | pending |
| `nc-t4-revision-drift` | model revision comparison | IOS-XE lab device (pending) | device-advertised module revisions diff against the committed lockfile; drift surfaces as warnings | pending |
