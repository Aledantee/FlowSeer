...
// renderModule produces the rendered (gofmt-clean) source for a single
// module. It does not touch the filesystem; callers compose write or
// diff behavior on top.
func renderModule(mod *gosmi.SmiModule, cm Module, cfgByName map[string]Module, pkgPrefix string) ([]byte, error) {
	...
	// Render to a buffer; jennifer is responsible for gofmt-equivalent
	// formatting and managed imports.
