...
```sh
task snmp:t1   # Net-SNMP snmpd smoke / USM matrix / forged-edge wire
task snmp:t2   # Nokia SR Linux end-to-end (requires containerlab)
task snmp:t3   # snmpsim replay against committed .snmprec baselines
SNMP_T4_TARGETS=host[:port]@community,... task snmp:t4   # live MikroTik / vendor device
```

Each `task snmp:t<N>` invocation builds a Docker image or deploys
...
1. Builds the Docker image from
   `common/snmp/integration/testdata/snmpd/Dockerfile` (debian + snmpd
...
4. Runs the tests in
   `common/snmp/integration/t1_*_test.go`:
...
```sh
containerlab deploy -t common/snmp/integration/testdata/containerlab/srlinux.clab.yaml --reconfigure
...
Tests in `common/snmp/integration/t2_*_test.go` exercise:
...
common/snmp/integration/scripts/capture-snmprec.sh \
    --target 10.0.0.5 \
    --community public \
    > common/snmp/integration/testdata/snmprec/cisco/ios-xe.snmprec

# 2. Append one entry to common/snmp/integration/testdata/snmprec/manifest.yaml:
#
#    - vendor: cisco
#      device: ios-xe-17
...
```sh
SNMP_T4_TARGETS="10.20.0.219@tegi,10.20.0.1@tegi" task snmp:t4
```
...
`task snmp:t<N>` target inside a runner with Docker (and `containerlab` for
