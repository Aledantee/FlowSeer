...
func TestLoad_LagCustomConfiguration(t *testing.T) {
	...
	cfg, _, _, err := netmodel.Load(t0, []*interfacev1.Interface{lagIface, p1, p2}, nil, nil, nil, nil, nil, []*lacpv1.AggregatorState{aggState}, nil, nil, nil, nil)
	if err != nil {
...
func TestLoad_LagDefaultBondMode(t *testing.T) {
	...
	t0 := time.Date(2026, 9, 11, 10, 0, 0, 0, time.UTC)
	cfg, _, report, err := netmodel.Load(t0, []*interfacev1.Interface{lagIface, p1}, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("Load failed: %v", err)
	}
...
func TestLacp_RoundTrip(t *testing.T) {
	...
	loadedCfg, _, _, err := netmodel.Load(t0, ifaces, nil, nil, nil, nil, nil, aggs, portStates, nil, nil, nil)
	if err != nil {
		t.Fatalf("Load failed: %v", err)
	}
