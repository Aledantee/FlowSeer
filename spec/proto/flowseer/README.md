...
spec/proto/flowseer/
  ...
  integration/   Fabric contracts between central and integrations (reserved)
  api/           Northbound Connect services for operators, apps, and workflows
...
embed. `event/`, `integration/`, and `api/` are boundary consumers that import
`model/`, `errs/`, and `net/` as needed, but never import each other. A package
that declares a Connect service is a sink and is imported by nothing. `store/`
