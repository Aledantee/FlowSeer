...
- **Requirements:** R10. 
- **Dependencies:** U1, U2, U3, U4.
- **Files:** all ae-importing files under `src/common/snmp/` (heaviest: `pdu.go`, `ber.go`, `v3msg.go`, `decode.go`, `session_engine.go`, `watch.go`, `watcher.go`, `tc.go`, `usm_*.go`, `backend.go`, `oid.go`, `reactor.go`, `errors.go`, `walker.go`, `options.go`, plus tests `tc_test.go`, `watch_test.go`, `oid_test.go`, `watcher_native_test.go`), `test/integration/snmp/` and its `testenv/`, `src/common/snmp/bench/netsnmp.go` (own module; `replace` to repo root already wired).
- **Approach:** per KTD6 —
