...
- **R8.** `generated/` is regenerated in the same change as each schema edit
  and `test/conformance/proto/` passes `go test -race`.
...
`docs/code-style-proto.md` Workflow section, `test/conformance/proto/`,
...
`test/conformance/proto/l2_rules_test.go`,
`test/conformance/proto/l3_rules_test.go`,
new `test/conformance/proto/addr_rules_test.go`,
regenerated `generated/go/proto/flowseer/**`.
**Approach:**
...
`go test -race ./test/conformance/proto/...` passes.
...
renumber reaches: `test/conformance/proto/addr_rules_test.go` (new in
...
3. `TestNeighborEntryReservedStateTag` in `l3_rules_test.go` is
   ...
   retires it. Because `test/conformance/proto/` is an AGENTS.md policy
...
`test/conformance/proto/layout_test.go` (README placement rules) and
...
- `test/conformance/proto/` is an AGENTS.md **policy surface**: U1's new
