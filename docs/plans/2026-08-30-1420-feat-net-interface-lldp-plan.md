...
- The rename is source- and wire-breaking for the generated `switching`/`ip` packages; accepted because no production code consumes them yet (verified — only `src/common/protoconformance/{l2,l3}_rules_test.go` import them, and those move with the rename).
...
- **Test scenarios (in `src/common/protoconformance/interface_rules_test.go`):**
...
- **Files:** `src/common/protoconformance/layering_test.go`.
...
- **Patterns to follow:** `src/common/protoconformance/layout_test.go` (spec-tree walking).
- **Test scenarios:**
...
- **Verification:** `go test -race ./src/common/protoconformance/`.
...
- **Patterns to follow:** fixture-driven fake session in `src/common/snmp/integration/assertions_test.go` (re-implemented locally per KTD2); `generated/go/mib/ifmib/mib.go` walker API.
- **Test scenarios:**
...
| Race tests | `go test -race ./src/common/protoconformance/ ./src/common/snmpmap/ ./src/common/snmp/...` | U1–U7 |
