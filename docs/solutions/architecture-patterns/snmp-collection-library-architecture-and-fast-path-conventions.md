...
severity: high
applies_when:
  - working anywhere under src/protocol/snmp or the generated MIB packages it feeds
...
`src/protocol/snmp` is not a wrapper around a wire library. It owns every byte on
...
(`src/protocol/snmp/pdu.go:13`); reactor (`reactor.go`) — one socket and one read loop
...
- **Byte-for-byte encoder equivalence.** `TestEncodeRequestFastEquivalence` builds
  ...
  non-null varbind (`src/protocol/snmp/rawwalk_test.go:325`). Accepting too much is
...
- **Validator/decoder mirror.** `validateRawValue` decides accept/reject without
  ...
  mirrored (`src/protocol/snmp/pdu.go:539`), pinned by
  `TestValidateRawValue_MirrorsDecodeValue` over every known tag plus three
  unknown ones (`src/protocol/snmp/rawwalk_test.go:164`).
...
(`src/protocol/snmp/test/integration/t3_offspec_test.go:21`).
