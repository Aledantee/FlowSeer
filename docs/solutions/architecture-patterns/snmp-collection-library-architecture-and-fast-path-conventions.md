...
- `raw-wrong-typed-column` — provenance `telegraf #14598; snmp_exporter #338`,
  ...
  `snmprec` replay (`src/common/snmp/integration/t3_offspec_test.go:29`; manifest
  entry `src/common/snmp/integration/testdata/snmprec/manifest.yaml:29`).
...
| Integration t1–t4 | `src/common/snmp/integration/` | tags `snmp_integration_t1..t4` |
...
(`src/common/snmp/integration/t3_offspec_test.go:21`).
...
- `src/common/snmp/integration/README.md` — per-tier prerequisites and
  walkthroughs.
- Stale prose to fix when next in the area: `src/common/snmp/README.md:68`
  ...
  and `src/common/snmp/integration/doc.go:8`, which says "Three independent
  tiers" and documents only t1–t3 while a build-tag-gated, opt-in t4 live-device
  tier exists (`src/common/snmp/integration/t4_main_test.go:1`).
