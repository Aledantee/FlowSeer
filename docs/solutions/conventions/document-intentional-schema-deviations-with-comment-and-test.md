...
`src/common/protoconformance/api_inventory_rules_test.go:36-42`, which
...
2. **Pin the loose behavior with an explicit conformance test case whose
   ...
   ```go
   // src/common/protoconformance/api_inventory_rules_test.go:36-42
   {
...
- `docs/code-style-proto.md` — states the comment-the-deviation norm for
  ...
  also places executable schema tests in `src/common/protoconformance/`.
