...
| [Virtual Device](2026-09-10-virtual-device-direction.md) | Proposed direction | Building or consuming a simulated device: the port table, the per-layer packages under `src/common/netsim`, frame forwarding over a typed configuration, or comparing a device's current and expected state. |
