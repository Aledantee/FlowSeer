...
`model/edge/v1`; `api/edge/v1` keeps `EdgeAdminService`; the enrollment,
heartbeat, and credential calls are `EdgeService` in `edge/attach/v1`.
`integration/device` now reads `edge/dispatch`; `event/device` split, its
`AuditService` into `edge/audit` and its `DeviceOperationEvent` into
`event/access`. See
