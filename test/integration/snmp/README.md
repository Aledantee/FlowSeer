...
```sh
task --dir test/integration/snmp t1   # Net-SNMP snmpd smoke / USM matrix / forged-edge wire
task --dir test/integration/snmp t2   # Nokia SR Linux end-to-end (requires containerlab)
task --dir test/integration/snmp t3   # snmpsim replay against committed .snmprec baselines
SNMP_T4_TARGETS=host[:port]@community,... task --dir test/integration/snmp t4   # live MikroTik / vendor device
```

Each `task --dir test/integration/snmp t<N>` invocation builds a Docker image or deploys
...
`task --dir test/integration/snmp t2`.
...
```sh
task --dir test/integration/snmp t1
```

The harness:

1. Builds the Docker image from
   `test/integration/snmp/testdata/snmpd/Dockerfile` (debian + snmpd
...
4. Runs the tests in
   `test/integration/snmp/t1_*_test.go`:
...
```sh
containerlab deploy -t test/integration/snmp/testdata/containerlab/srlinux.clab.yaml --reconfigure
...
Tests in `test/integration/snmp/t2_*_test.go` exercise:
...
```sh
task --dir test/integration/snmp t3
```
...
test/integration/snmp/scripts/capture-snmprec.sh \
    --target 10.0.0.5 \
    --community public \
    > test/integration/snmp/testdata/snmprec/cisco/ios-xe.snmprec

# 2. Append one entry to test/integration/snmp/testdata/snmprec/manifest.yaml:
#
#    - vendor: cisco
#      device: ios-xe-17
...
```sh
SNMP_T4_TARGETS="10.20.0.219@tegi,10.20.0.1@tegi" task --dir test/integration/snmp t4
```
...
`task --dir test/integration/snmp t<N>` target inside a runner with Docker (and `containerlab` for
